 ...
 * HeapPartition objects which are themselves NavigableSets used as heaps.
 *
 * The consistency of this data structure is very loose--operations that combine the results on the partitions are
 * not well-defined. Operations per-partition are synchronized so that operations on them are consistent. This includes
 * add(), ceiling(), floor(), higher(), lower(), remove(), removeAll(). last() and pollLast() function similarly
 * to isEmpty()/size() in that operations that modify the data after it starts may be ignored.
 *
 *
 * An example is isEmpty(). Since this iterates on each partition, it's possible to observe each partition as empty, but
 * at no moment was the entire set empty. If precise values are required for aggregate functions such as isEmpty()
 * or size(), use your own locking mechanism (eg a read/write lock)
 *
 * @param <T> the upper bound type of each OrderedSet
 */
...
public class PartitionedOrderedSet<T extends Comparable<? super T>> implements OrderedSet<T> {
  ...
  /**
   * finds the index of the set that should contain value (either actually will contain, or can be placed here)
   *
   * this does not need to be synchronized because the upperBound of each set does not change, and that is what we are
   * searching on
   *
   * @param value search critiera
   * @return
   */
