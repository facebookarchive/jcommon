...
public class TestThreadLocalThenMostFreePolicy {
  ...
  public void testAllocateAcrossSlabsMax() throws Exception {
    long address1 = policy.allocate(slab -> slab.allocate(Sizes.MB.ov(2)));
    long address2 = policy.allocate(slab -> slab.allocate(Sizes.MB.ov(2)));
    long address3 = policy.allocate(slab -> slab.allocate(Sizes.MB.ov(1)));
    long address4 = policy.allocate(slab -> slab.allocate(Sizes.MB.ov(1)));
