...
public class TestLinkedListBucket {
  ...
  private LinkedListBucketNodeAccessorWithLruCache linkedListBucketNodeAccessor;
  ...
  public void setUp() throws Exception {
    ...
    linkedListBucketNodeAccessor = new LinkedListBucketNodeAccessorWithLruCache();
  }
  ...
  public void testPutAndConstructors() throws FailedAllocationException {
    Bucket list = LinkedListBucket.create(
      slab,
      memoryViewFactory,
      valueWrapper,
      keyWrapper,
  ...
  public void testPutsAndRemoves1() throws FailedAllocationException {
    Bucket bucket = LinkedListBucket.create(
      slab,
      memoryViewFactory,
      valueWrapper,
      keyWrapper,
    ...
    Assert.assertEquals(bucket.length(), 3);
    bucket.remove(key2);
    Assert.assertEquals(bucket.length(), 2);
  ...
  public void testSingleNode() throws FailedAllocationException {
    ...
    Assert.assertEquals(bucket.length(), 0);
  }

  @Test
  public void testEmptyBucket() throws Exception {
    Bucket sourceBucket = LinkedListBucket.create(
      slab,
      memoryViewFactory,
      valueWrapper,
      keyWrapper,
    ...
    Assert.assertEquals(sourceBucket.length(), 0);

    long address = sourceBucket.getAddress();

    Bucket wrappedBucket = LinkedListBucket.wrap(
      address,
      slab,
      memoryViewFactory,
      valueWrapper,
    ...
    Assert.assertEquals(wrappedBucket.length(), 0);
  }
}
