...
public class TestOffHeapCacheEntry {
  ...
  public void testSlots() throws Exception {
    Struct offHeapCacheEntryStruct = Structs.getStruct(OffHeapCacheEntry.class);
    ...
    Assert.assertEquals(nextField.getOffset(), 0);
    Assert.assertEquals(dataField.getFieldType(), FieldType.ADDRESS);
    Assert.assertEquals(dataField.getOffset(), 8);
    Assert.assertEquals(previousField.getFieldType(), FieldType.ADDRESS);
    Assert.assertEquals(previousField.getOffset(), 16);
    Assert.assertEquals(offHeapCacheEntryStruct.getOffset(), 24);
  }
