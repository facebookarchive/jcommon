...
public class TestSpanNode {
  ...
  public void testLinkedList() throws Exception {
    MemoryView head = SpanNode.DEF.allocate(slab1);
    MemoryView data = MemoryViewMediator.fromSlab(slab2).allocate32(Long.BYTES);
...
//      dataPtr = slab2.allocate(Long.SIZE);
      data = MemoryViewMediator.fromSlab(slab2).allocate32(Long.SIZE);
      data.putLong(0, i*i);
      MemoryView next = spanNode.allocate(slab1);
...
//      System.err.println(SpanNode.PREVIOUS.get(ptr));
      MemoryView dataView = MemoryView32.factory().create(span.dataAddress().get(ptr), Long.BYTES);
