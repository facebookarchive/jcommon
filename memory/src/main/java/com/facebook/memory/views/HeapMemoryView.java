...
public class HeapMemoryView implements MemoryView {
  ...
  @Override
  public MemoryView reset() {
    return new HeapMemoryView(bytes, 0);
  }

  @Override
  public MemoryView splice(long offset) {
    Preconditions.checkArgument(offset < Integer.MAX_VALUE);

    return new HeapMemoryView(Arrays.copyOfRange(bytes, (int) offset, bytes.length), 0);
  }
  ...
  public MemoryView splice(long offset, long size) {
    ...
    return new HeapMemoryView(Arrays.copyOfRange(bytes, (int) offset, (int) size), 0);
  }
  ...
  public static class Factory extends AbstractMemoryViewFactory {
    ...
    public MemoryView wrap(long address, long size) {
      ...
      return new HeapMemoryView(new byte[(int) size], 0);
    }

    public ReadableMemoryView create(int size) {
      return new HeapMemoryView(new byte[size], 0);
    }

    public ReadableMemoryView create(byte[] bytes) {
      return new HeapMemoryView(bytes, 0);
    }
