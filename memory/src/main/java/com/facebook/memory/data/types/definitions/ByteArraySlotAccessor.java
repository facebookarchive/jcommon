...
public class ByteArraySlotAccessor extends AbstractSlotAccessor {
  ...
  ByteArraySlotAccessor(long baseAddress, int length, FieldOffsetMapper fieldOffsetMapper) {
    super(baseAddress, length + Integer.BYTES, fieldOffsetMapper);
  }

  ByteArraySlotAccessor(
  ...
  /**
   * both wrap() methods assume that lenght is already stored in the off-heap structure
   * @param baseAddress
