...
import com.facebook.collections.heaps.PartitionedHeap;
import com.facebook.memory.FailedAllocationException;
import com.facebook.memory.data.structures.LargestRangeExtractor;
import com.facebook.memory.data.structures.RangeExtractor;
...
public class TreeSetFreeList implements FreeList {
  ...
  private final Heap<IntRange> freeRangeBySize;
  ...
  public TreeSetFreeList(int size, int minPartitionSize, RangeExtractor rangeExtractor) {
    ...
    freeRangeBySize = new PartitionedHeap<>(
  ...
  public synchronized void free(int offset, int size) {
    ...
    Preconditions.checkState(
      ...
      "range [%d] overlaps with existing [%d]",
      range,
      higherRange
    );
  ...
  public synchronized Span tryAllocate(int size) {
    if (freeRangeBySize.isEmpty()) {
      ...
      if (range == null) {
        ...
        int actualAllocation = Math.min(range.getSize(), 0);
