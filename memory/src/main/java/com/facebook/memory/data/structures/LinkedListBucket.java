...
public class LinkedListBucket implements Bucket {
  ...
  public static LinkedListBucket wrap(
    ...
    return new LinkedListBucket(
      address,
      slab,
      memoryViewFactory,
      keyWrapper,
      valueWrapper,
      linkedListBucketNodeAccessor
    );
  }
  ...
  public BucketPutResult put(SizedOffHeapStructure key, SizedOffHeapStructure value) throws FailedAllocationException {
    ...
    for (int i = 0; i < size; i++) {
      if (curr.keyEquals(key)) {
        SizedOffHeapStructure existingValue = curr.getValue();

        curr.replaceValue(value);

        return BucketPutResult.createExistingEntry(
          new AnnotatedOffHeapValue(existingValue, curr.getAnnotationAddress())
        );
      }
  ...
  private AnnotatableMemoryAddress insertAtHead(SizedOffHeapStructure key, SizedOffHeapStructure value)
    throws FailedAllocationException {
    final LinkedListBucketNode curr = linkedListBucketNodeAccessor.create(slab, key, value, keyWrapper, valueWrapper);
    if (head == null) {
      head = tail = curr;
      ...
      tail = curr;
      tmp.setNextAddress(tail.getAddress());
    }
    size++;
  ...
  public boolean remove(SizedOffHeapStructure key) {
    if (head.keyEquals(key)) {
      LinkedListBucketNode tmp = size > 1 ? head.nextNode() : null;
  ...
  private void removeEntry(LinkedListBucketNode curr, LinkedListBucketNode prev, boolean isHead) {
    if (isHead) {
      ...
      LinkedListBucketNode next = curr.getAddress() == tail.getAddress() ? null : curr.nextNode();
      prev.setNextAddress(next == null ? MemoryConstants.NO_ADDRESS : next.getAddress());
      size--;
      memoryView.putInt(SIZE_OFFSET, size);
      if (next == null) {
        tail = prev;
