...
/**
 * Keeps a list of gauge counters (see AbstractCompositeCounter) that look like one summarized gauge
 * counter.
 */
public class CompositeGaugeCounter extends AbstractCompositeSum<GaugeCounter>
  ...
  public void add(long delta, long nsamples) {
    ...
    synchronized (this) {
      if (getMostRecentCounter() == null || !now.isBefore(getMostRecentCounter().getEnd())) {
    ...
    while (counterIterator.hasNext()) {
      ...
      if (first) {
        ...
        if (getWindowStart().isAfter(counter.getStart())) {
          ...
          nsamples -= (long) (getExpiredFraction(counter) * counter.getSamples());
        }
        first = false;
      }
    }
