...
public interface CompositeEventCounterIf<C extends EventCounterIf<C>> extends EventCounterIf<C> {
   ...
   */
  public CompositeEventCounterIf<C> add(long delta, ReadableDateTime start, ReadableDateTime end);

  /**
   * Add an already created counter to the ComopsiteEvenCountIf. This is similar to {@link
   * #add(long, org.joda.time.ReadableDateTime, org.joda.time.ReadableDateTime)} but externalizes
   * creation of the counter to add. ie, you can add another CompositeEventCounterIf if C so allows
   * it, allowing for nesting of composition to arbitrary depths.
   *
