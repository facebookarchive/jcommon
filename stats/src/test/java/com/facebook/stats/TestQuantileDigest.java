...
public class TestQuantileDigest {
  private static final Logger LOG = LoggerImpl.getLogger(TestQuantileDigest.class);

  /**
   * This value should be the result of:
   *
   * <p>use the same value in all cases, result of calling addTestValuesForHistogram on TestingClock
   * testingClock = new TestingClock(System.currentTimeMillis()); MultiWindowDistribution
   * multiWindowDistribution = new MultiWindowDistribution( new QuantileDigest(maxError, 0.0,
   * testingClock, false), new QuantileDigest(maxError, 0.0, testingClock, false), new
   * QuantileDigest(maxError, 0.0, testingClock, false), new QuantileDigest(maxError, 0.0,
   * testingClock, false) );
   *
   * <p>addTestValuesForHistogram(multiWindowDistrobution);
   */
  ...
  public void setUp() throws Exception {
    ...
    multiWindowDistributionForCounterTests =
        new MultiWindowDistribution(
            new QuantileDigest(maxError, 0.0, testingClock, false),
            new QuantileDigest(maxError, 0.0, testingClock, false),
            new QuantileDigest(maxError, 0.0, testingClock, false),
            new QuantileDigest(maxError, 0.0, testingClock, false));
    multiWindowDistributionForHistogramTests =
        new MultiWindowDistribution(
            new QuantileDigest(maxError, 0.0, testingClock, false),
            new QuantileDigest(maxError, 0.0, testingClock, false),
            new QuantileDigest(maxError, 0.0, testingClock, false),
  ...
  public void testHistogramQuery() throws Exception {
    ...
    assertEquals(
        ...
        asList(new QuantileDigest.Bucket(7, 3), new QuantileDigest.Bucket(3, 8)));
    ...
    assertEquals(
        digest.getHistogram(asList(Long.MAX_VALUE)), asList(new QuantileDigest.Bucket(10, 4.5)));
  }
  ...
  public void testDecayedQuantiles() throws Exception {
    ...
    QuantileDigest digest =
        new QuantileDigest(1, ExponentialDecay.computeAlpha(0.5, 60), clock, true);
    ...
    // Considering that the first 10 values now have a weight of 0.5 per the alpha factor, they only
    // contributed a count
    // of 5 to rank computations. Therefore, the 50th percentile is equivalent to a weighted rank of
    // (5 + 10) / 2 = 7.5,
  ...
  public void testDecayedCountsWithClockIncrementSmallerThanRescaleThreshold() throws Exception {
    ...
    TestingClock clock = new TestingClock();
    QuantileDigest digest =
        new QuantileDigest(1, ExponentialDecay.computeAlpha(0.5, targetAgeInSeconds), clock, false);
  ...
  public void testRescaleWithDecayKeepsCompactTree() throws Exception {
    ...
    QuantileDigest digest =
        new QuantileDigest(
            0.01,
            ExponentialDecay.computeAlpha(
                QuantileDigest.ZERO_WEIGHT_THRESHOLD / 2, targetAgeInSeconds),
            clock,
            true);
  ...
  public void testTiming() {
    ...
    LOG.info(
        "Compressions: %d, %f entries/compression",
