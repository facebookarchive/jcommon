...
/**
 * Represents a time interval type when specifying a time period. Instances of this class provide
 * means to calculate the start instant of the interval containing a given time instant.
 *
 * <p>Each interval type computes the time interval assuming that they start from it's minimum
 * value. For example, MINUTE represents the time interval starting from the 0th minute in the
 * containing hour. So given a time instant 2011-10-09T13:11:49, and a time interval type of MINUTE
 * and length 3, the start instant of the time interval containing that instant will be
 * 2011-10-09T13:09:00.
 */
public enum TimeIntervalType {
  ...
  /**
   * Week interval is currently non-intuitive. It assumes that the weeks start on the same day as
   * the first day of the year. Don't use weeks as a time interval type, until we can have the weeks
   * starting on the correct day (Sunday/Monday) per the locale.
   */
  ...
  /**
   * Validates that the specified interval value is valid for this interval type in the supplied
   * time zone.
   *
  ...
  /**
   * Gets the start instant given the event instant, interval length and the time zone for this
   * interval type.
   *
   * @param instant the event time instant.
   * @param length the interval length
   * @return the start instant of the interval of given length that contains the supplied time
   *     instant in the supplied time zone
   */
  public DateTime getTimeIntervalStart(DateTime instant, long length) {
    validateValue(instant.getZone(), length);

    // Reset all the fields
    DateTime periodStart = instant.property(truncateFieldType).roundFloorCopy();
