...
public class TestCoreConcurrentCache {
  ...
  public void setUp() throws Exception {
    stdProducer = new BlockingValueProducer<String, RuntimeException>(VALUE);
    blockingProducer = new BlockingValueProducer<String, RuntimeException>(VALUE, true);
    exceptionThrowingProducer =
      new BlockingValueProducer<String, RuntimeException>(
        VALUE, true, new RuntimeException("I take exception to this")
      );
    cache = new CoreConcurrentCache<String, String, RuntimeException>(
      new ValueFactory<String, String, RuntimeException>() {
        @Override
        public String create(String input) throws RuntimeException {
          if (input.equals(STD_KEY)) {
            return stdProducer.call();
          } else if (input.equals(BLOCKING_KEY)) {
            return blockingProducer.call();
          } else if (input.equals(EXCEPTION_KEY)) {
    ...
    testHelper = new ConcurrentCacheTestHelper<String, String>(cache);
  }
