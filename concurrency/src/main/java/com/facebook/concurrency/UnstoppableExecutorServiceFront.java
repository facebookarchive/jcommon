...
/**
 * combines the functionality of an ExecutorServiceFront and UnstoppableExecutorService. The net
 * effect is a virtual executor over another. You bound the number of 'virtual threads' (Drainers)
 * and optionally a max time slice each Drainer may use. This is approximate as a Drainer does not
 * interrupt tasks running--it only checks between tasks to see if the maxTimeSlice is reached and
 * then gives up the underlying thread in the delegate executor
 */
public class UnstoppableExecutorServiceFront extends UnstoppableExecutorService {
   ...
   * @param maxTimeSlice after every task, a drainer will check and see if the maxTimeSlice has
   *     passed and will terminate, but re-submit itself into the executor ('fair-share' the
   *     underlying executor)
  ...
  public UnstoppableExecutorServiceFront(
    ...
    super(
        new ExecutorServiceFront(
            workQueue, executor, poolName, maxDrainers, maxTimeSlice, maxTimeSliceUnit));
  }
  ...
  public UnstoppableExecutorServiceFront(
      ...
      TimeUnit maxTimeSliceUnit) {
    super(
        new ExecutorServiceFront(workQueue, executor, maxDrainers, maxTimeSlice, maxTimeSliceUnit));
  }
